...
advancedvectorizer = TfidfVectorizer(tokenizer=tokenize,max_features = 10000,stop_words='english')  #TfidfVectorizer
...
# Here's a Multi-Layer-Perceptron Model(MLP) 
model = Sequential()
...
model.fit(X_train, Y_train, nb_epoch=20, batch_size=100,validation_data=(X_test, Y_test))
...
output.to_csv( "MLPoutput.csv", index=False, quoting=3 )
